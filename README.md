...
The live supervisor now runs as a durable mission worker for `control_supervisor` tasks: each iteration is selected from the persisted scheduler queue, resumed after Gateway restart when the parent task is still `running`, and terminated as `mission_aborted:*` when a Mission Contract abort condition such as `human approval required` matches. Successful current-tab checks link `current_tab_info` and verifier reports into the durable task graph so the Control UI can show the Mission Contract, queue state, checkpoints, evidence refs, and abort reason from the same task artifact. This runtime intentionally remains supervisor-owned; a general multi-node mission scheduler, paginated startup resume, stricter queue prioritization, and typed abort condition enums are follow-on work.
